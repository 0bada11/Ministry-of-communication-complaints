...
# real work on the dashboard rather than an empty queue.
SAMPLES = [
    ...
    ("تعذّر إتمام معاملة إلكترونية على البوابة",
     "لم تكتمل المعاملة الإلكترونية عند تأكيد الدفع الإلكتروني، مع خصم المبلغ من الحساب."),
    ...
    ("ضعف تغطية الشبكة في قرية نائية",
     "لا توجد تغطية كافية في المنطقة ويحتاج الأمر إلى برج جديد أو توسعة الشبكة القائمة."),
    ...
    ("تعذّر تسجيل الدخول إلى البوابة الحكومية",
     "المنصة الحكومية لا تقبل كلمة المرور الصحيحة وتعطي خطأ في كل محاولة تسجيل دخول."),
    ("اشتباه تسريب بيانات من نظام حكومي",
     "وصلتني رسائل تصيد تحمل بياناتي الشخصية، وأشتبه بوجود تسريب أو اختراق لأحد الأنظمة."),
    ("معاملة ما تزال ورقية وتحتاج مراجعة شخصية",
     "أطلب رقمنة هذه المعاملة وأتمتة إجراءاتها، فالربط بين الجهات الحكومية ما يزال ضعيفاً."),
    ("عدم تطابق البيانات بين نظامين حكوميين",
     "قاعدة البيانات لدى جهتين تعطي سجلاً مختلفاً لنفس الشخص، ويظهر خلل في التكامل بين الأنظمة."),
    ("أرقام غير دقيقة في لوحة المؤشرات المنشورة",
     "المؤشرات المنشورة تحتوي بيانات ناقصة ولا تطابق التقرير الإحصائي السابق."),
    ...
    ("Suspicious login activity on a government portal",
     "I received a phishing message and suspect a security breach on my government account."),
]
